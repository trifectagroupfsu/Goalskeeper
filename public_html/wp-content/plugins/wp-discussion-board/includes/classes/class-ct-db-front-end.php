...
if( ! class_exists( 'CT_DB_Front_End' ) ) {
	...
	class CT_DB_Front_End {
		...
		public function new_topic_form_content() {
			...
			$form['new_topic_message'] = $options['new_topic_message'];
			
			$form['open_wrap'] = '<div id="ctdb-new-topic">';
			$form['open_form'] = '<form id="new-topic" name="new-topic" method="post" action="">';
			$form['fields'] = array();
			$form['fields']['title'] = '<input class="required" type="text" id="topic_title" value="' . esc_attr( $title ) . '" tabindex="900" name="topic_title" placeholder="Topic Title" />';
			$form['fields']['content'] = '<textarea id="topic_content" name="topic_content" cols="80" rows="20" tabindex="901" placeholder="Topic Content">' . wp_kses_post( $content ) . '</textarea>';
								...
								if( isset( $titles[$col] ) ) {
									...
									$body_fields[$post->ID] .= '<li class="ctdb-topic-table-item ctdb-topic-table-' . esc_attr( $col ) . '">';
										if( $col == 'avatar' ) {
											...
											$body_fields[$post->ID] .= get_comments_number();
										} else if( $col == 'started' ) {
											$body_fields[$post->ID] .= '<time class="comment-timeago" datetime="' . date( 'c', strtotime( get_the_date() . ' ' . get_the_time() ) ) . '">' . get_the_date() . '</time>';
										} else if( $col == 'voices' ) {
		...
		public function return_access_restricted_message() {
			...
			if( ! is_user_logged_in() && ! is_archive() ) {				
				...
				$message = '<p class="ctdb-restricted-message">' . __( 'You\'re logged in but you don\'t seem to have permission to view the content. Please contact the site administrator for further information.', 'wp-discussion-board' ) . '</p>';
			}
			return $message;		
		}
